structure GLibTimeZone :>
  ...
  struct
    ...
    val getAbbreviation_ = fn x1 & x2 => (_import "g_time_zone_get_abbreviation" : GLibTimeZoneRecord.FFI.notnull GLibTimeZoneRecord.FFI.p * GInt.FFI.val_ -> Utf8.FFI.notnull Utf8.FFI.out_p;) (x1, x2)
    val getOffset_ = fn x1 & x2 => (_import "g_time_zone_get_offset" : GLibTimeZoneRecord.FFI.notnull GLibTimeZoneRecord.FFI.p * GInt.FFI.val_ -> GInt32.FFI.val_;) (x1, x2)
    val isDst_ = fn x1 & x2 => (_import "g_time_zone_is_dst" : GLibTimeZoneRecord.FFI.notnull GLibTimeZoneRecord.FFI.p * GInt.FFI.val_ -> GBool.FFI.val_;) (x1, x2)
    ...
    fun getAbbreviation self interval = (GLibTimeZoneRecord.FFI.withPtr &&&> GInt.FFI.withVal ---> Utf8.FFI.fromPtr 0) getAbbreviation_ (self & interval)
    fun getOffset self interval = (GLibTimeZoneRecord.FFI.withPtr &&&> GInt.FFI.withVal ---> GInt32.FFI.fromVal) getOffset_ (self & interval)
    fun isDst self interval = (GLibTimeZoneRecord.FFI.withPtr &&&> GInt.FFI.withVal ---> GBool.FFI.fromVal) isDst_ (self & interval)
  end
