structure CairoRectangleIntRecord :>
  sig
    include CAIRO_RECTANGLE_INT_RECORD
  end =
  struct
    type notnull = CPointer.notnull
    type 'a p = 'a CPointer.p

    val new_ = _import "giraffe_cairo_rectangle_int_t_new" : unit -> notnull p;

    val copy_ = _import "giraffe_cairo_rectangle_int_t_copy" : notnull p -> notnull p;

    val free_ = _import "giraffe_cairo_rectangle_int_t_free" : notnull p -> unit;
    ...
    val getType_ =
      _import "cairo_gobject_rectangle_int_get_type" : unit -> GObjectType.C.val_;
