structure CairoRectangleIntRecord :> CAIRO_RECTANGLE_INT_RECORD =
  struct
    ...
    local
      open PolyMLFFI
    in
      val new_ = call (getSymbol "giraffe_cairo_rectangle_int_new") (cVoid --> cPtr)
      val copy_ = call (getSymbol "giraffe_cairo_rectangle_int_copy") (cPtr &&> cPtr --> cVoid)
      val free_ = call (getSymbol "giraffe_cairo_rectangle_int_free") (cPtr --> cVoid)
      val size_ = call (getSymbol "giraffe_cairo_rectangle_int_size") (cVoid --> GUInt.PolyML.cVal)
    end
    structure Record =
      BoxedValueRecord(
