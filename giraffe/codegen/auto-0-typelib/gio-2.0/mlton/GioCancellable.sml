structure GioCancellable :>
  ...
  struct
    ...
    val disconnect_ = fn x1 & x2 => (_import "g_cancellable_disconnect" : GObjectObjectClass.C.notnull GObjectObjectClass.C.p * FFI.UInt64.C.val_ -> unit;) (x1, x2)
    val getFd_ = _import "g_cancellable_get_fd" : GObjectObjectClass.C.notnull GObjectObjectClass.C.p -> FFI.Int32.C.val_;
    ...
    fun setErrorIfCancelled self = (GObjectObjectClass.C.withPtr &&&> GLibErrorRecord.C.handleError ---> FFI.Bool.C.fromVal) setErrorIfCancelled_ (self & [])
    local
