structure GioMenuItem :>
  ...
  struct
    ...
    val newFromModel_ = fn x1 & x2 => (_import "g_menu_item_new_from_model" : GioMenuModelClass.FFI.notnull GioMenuModelClass.FFI.p * GInt32.FFI.val_ -> GioMenuItemClass.FFI.notnull GioMenuItemClass.FFI.p;) (x1, x2)
    val newSection_ =
      fn
        (x1, x2) & x3 =>
          (
    ...
    fun newFromModel (model, itemIndex) = (GioMenuModelClass.FFI.withPtr &&&> GInt32.FFI.withVal ---> GioMenuItemClass.FFI.fromPtr true) newFromModel_ (model & itemIndex)
