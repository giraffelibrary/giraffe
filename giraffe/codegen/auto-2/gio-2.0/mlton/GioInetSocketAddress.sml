structure GioInetSocketAddress :>
  ...
  struct
    ...
    val new_ = fn x1 & x2 => (_import "g_inet_socket_address_new" : GObjectObjectClass.C.notnull GObjectObjectClass.C.p * FFI.UInt16.val_ -> GObjectObjectClass.C.notnull GObjectObjectClass.C.p;) (x1, x2)
    ...
    val getPort_ = _import "g_inet_socket_address_get_port" : GObjectObjectClass.C.notnull GObjectObjectClass.C.p -> FFI.UInt16.val_;
    ...
    fun new address port = (GObjectObjectClass.C.withPtr &&&> FFI.UInt16.withVal ---> GioInetSocketAddressClass.C.fromPtr true) new_ (address & port)
    fun getAddress self = (GObjectObjectClass.C.withPtr ---> GioInetAddressClass.C.fromPtr false) getAddress_ self
    fun getPort self = (GObjectObjectClass.C.withPtr ---> FFI.UInt16.fromVal) getPort_ self
    local
      open Property
    in
      val addressProp =
