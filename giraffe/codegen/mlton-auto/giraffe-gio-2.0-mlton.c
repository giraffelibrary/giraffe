...
mlton_g_settings_set_strv (GSettings* settings,
                           SML_CVECTOR_VAL(const gchar, key),
                           SML_CVECTORVECTOR_VAL(gchar, value))
{
  return g_settings_set_strv (settings,
                              GET_SML_CVECTOR_VAL(const gchar, key),
                              GET_SML_CVECTORVECTOR_VAL(gchar, value));
}
#endif
...
#if GLIB_CHECK_VERSION(2, 40, 0)
GSubprocess*
mlton_g_subprocess_newv (SML_CVECTORVECTOR_VAL(gchar, argv),
                         GSubprocessFlags flags,
                         GError** error)
{
  return g_subprocess_newv (GET_SML_CVECTORVECTOR_VAL(gchar, argv),
                            flags,
                            error);
}
#endif
