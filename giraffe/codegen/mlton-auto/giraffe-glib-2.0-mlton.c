...
mlton_g_key_file_load_from_dirs (GKeyFile* key_file,
                                 SML_CVECTOR_VAL(gchar, file),
                                 SML_CVECTORVECTOR_VAL(gchar, search_dirs),
                                 SML_CVECTOR_REF(gchar, full_path),
                                 GKeyFileFlags flags,
                                 GError** error)
{
  return g_key_file_load_from_dirs (key_file,
                                    GET_SML_CVECTOR_VAL(gchar, file),
                                    GET_SML_CVECTORVECTOR_VAL(gchar, search_dirs),
                                    GET_SML_CVECTOR_REF(gchar, full_path),
                                    flags,
                                    error);
}
...
#if GLIB_CHECK_VERSION(2, 26, 0)
GVariant*
mlton_g_variant_new_bytestring_array (SML_CVECTORVECTOR_VAL(gchar, strv),
                                      gssize length)
{
  return g_variant_new_bytestring_array (GET_SML_CVECTORVECTOR_VAL(gchar, strv),
                                         length);
}
#endif
...
#if GLIB_CHECK_VERSION(2, 30, 0)
GVariant*
mlton_g_variant_new_objv (SML_CVECTORVECTOR_VAL(gchar, strv),
                          gssize length)
{
  return g_variant_new_objv (GET_SML_CVECTORVECTOR_VAL(gchar, strv),
                             length);
}
#endif
...
#if GLIB_CHECK_VERSION(2, 24, 0)
GVariant*
mlton_g_variant_new_strv (SML_CVECTORVECTOR_VAL(gchar, strv),
                          gssize length)
{
  return g_variant_new_strv (GET_SML_CVECTORVECTOR_VAL(gchar, strv),
                             length);
}
#endif
...
GVariantType*
mlton_g_variant_type_new_tuple (SML_CVECTOR_VAL(GVariantType*, items),
                                gint length)
{
  return g_variant_type_new_tuple (GET_SML_CVECTOR_VAL(GVariantType*, items),
                                   length);
}
