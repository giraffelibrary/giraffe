...
fun getParInfo
  ...
  let
    ...
    fun resolveType optIRef cxtPtrDepth (dir, mayBeNull, ownXfer) typeInfo =
      let
        open TypeTag
...
local
  ...
  fun withFunUtf8 (dir, {isPtr, xferOwn, isOpt, optIRef, ...} : utf8_info) =
    let
      val prefixIds =
        case optIRef of
 ...
 *)
  ...
  fun fromFunUtf8 (_, {xferOwn, isOpt, optIRef, ...} : utf8_info) =
    let
      val prefixIds =
        case optIRef of
          NONE      => [utf8StrId, ffiStrId]
...
local
  ...
  fun parUtf8Conv (dir, {isPtr, isOpt, optIRef, ...} : utf8_info) =
    let
      val prefixIds =
        case optIRef of
...
local
  ...
  fun parUtf8Type (dir, {isPtr, isOpt, optIRef, ...} : utf8_info) =
    let
      val prefixIds =
        case optIRef of
          NONE      => [utf8StrId, ffiStrId]
